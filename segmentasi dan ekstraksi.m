...
for i = 1:total_kelas % perulangan untuk i = 1 hingga 6 (total kelas)
    for j = 1:total_images % perulangan untuk j = 1 hingga 15 (total gambar)
        ...
        stats = regionprops(gray, 'Perimeter', 'Area', 'Eccentricity'); % menghitung fitur bentuk gambar n.gif berdasarkan daerah pada gambar
...
function [gray, RGB] = segmentasi(I)
    HSV = rgb2hsv(I);
    S = HSV(:, :, 2);
    level = graythresh(S);
